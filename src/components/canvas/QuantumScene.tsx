...
const MacroscopicWave = () => {
  ...
  return (
    <Torus ref={ref} args={[3, 0.1, 16, 100]} rotation={[Math.PI / 2, 0, 0]}>
      {/* Fixed: Property 'meshStandardMaterial' does not exist on type 'JSX.IntrinsicElements' */}
      <MeshStandardMaterial color="#5E5CE6" emissive="#5E5CE6" emissiveIntensity={0.5} transparent opacity={0.6} wireframe />
    </Torus>
  );
}
  ...
  return (
    <div className="absolute inset-0 z-0 opacity-60 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        {/* Fixed: Property 'ambientLight' does not exist on type 'JSX.IntrinsicElements' */}
        <AmbientLight intensity={0.5} />
        {/* Fixed: Property 'pointLight' does not exist on type 'JSX.IntrinsicElements' */}
        ...
        <Float speed={2} rotationIntensity={0.5} floatIntensity={1}>
          ...
          <QuantumParticle position={[3, -1, -3]} color="#5E5CE6" scale={0.6} />
        </Float>
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      <Canvas camera={{ position: [0, 0, 4.5], fov: 45 }}>
        ...
        {/* Fixed: Property 'spotLight' does not exist on type 'JSX.IntrinsicElements' */}
        <SpotLight position={[5, 5, 5]} angle={0.3} penumbra={1} intensity={2} color="#5E5CE6" />
        {/* Fixed: Property 'pointLight' does not exist on type 'JSX.IntrinsicElements' */}
        <PointLight position={[-5, -5, -5]} intensity={0.5} />
        <Environment preset="studio" />

        <Float rotationIntensity={0.4} floatIntensity={0.2} speed={1}>
          {/* Fixed: Property 'group' does not exist on type 'JSX.IntrinsicElements' */}
          <Group rotation={[0, 0, 0]} position={[0, 0.5, 0]}>
            {/* Main Cryostat Structure (Gold Chandelier) */}

            {/* Top Plate */}
            <Cylinder args={[1.2, 1.2, 0.1, 64]} position={[0, 1, 0]}>
              <MeshStandardMaterial color="#5E5CE6" metalness={1} roughness={0.15} />
            </Cylinder>
